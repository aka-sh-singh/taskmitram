...
import { Send, Loader2, LayoutDashboard, Workflow, CheckCircle2 } from 'lucide-react';
...
const ChatWindow = () => {
    ...
    }, [chatId, isAuthenticated, isStreaming]);
                    ...
                    <div className="max-w-3xl mx-auto space-y-6">
                        {messages.map((message) => {
                            ...
                            return (
                                ...
                                >
                                    <div
                                        className={`max-w-[80%] rounded-2xl px-4 py-3 transition-all duration-200 ${message.sender === 'user'
                                            ? 'bg-bubble-user text-primary-foreground'
                                            : 'bg-bubble-agent text-foreground shadow-sm'
                                            }`}
                                    >
                                        {message.sender === 'user' ? (
                                            ...
                                            <div className="space-y-4">
                                                <div className={`prose prose-sm dark:prose-invert max-w-none break-words font-sans content-transition ${isStreaming && message.id === messages[messages.length - 1]?.id ? 'streaming-content' : ''
                                                    }`}>
                                                    <ReactMarkdown>
                                                ...
                                                {message.content.match(/\[ACTION_ID:([^:]+):([^\]]+)\]/) && (() => {
                                                    ...
                                                    return (
                                                        <div className="mt-4 p-4 border border-primary/20 rounded-xl bg-primary/5 space-y-3 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        {isStreaming && (!messages[messages.length - 1]?.content) && (
                            <div className="flex justify-start">
                                <div className="bg-bubble-agent rounded-2xl px-4 py-3">
